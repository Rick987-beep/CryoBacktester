#!/usr/bin/env python3
"""Extract run 105 / combo a1276d6ce476 and build marketing assets."""
...
BUNDLE = Path("backtester/reports/short_str_turb_dyn_20260623_073150.bundle")
COMBO_IDX = 71
COMBO_HASH = "a1276d6ce476"
OUT = Path("analysis/run105_marketing")
...
def _return_pct_series(daily: list, capital: float) -> tuple[list[str], list[float], list[float], list[float]]:
    """Return dates and cumulative return % (0% at start, +59% = +59% gain)."""
...
def _svg_to_png(svg_path: Path, png_path: Path, width: int, height: int) -> None:
    """Render *svg_path* to *png_path* via headless Chrome (macOS)."""
    import subprocess
...
def equity_chart_svg(
    ...
    *,
    ...
    fs_dd = fs
    label_x = int(fs * 3.2)          # right edge of Y-axis label column
    ml = label_x + int(fs * 0.45)    # plot area starts after labels
    ...
    if eq_section_label:
        parts.append(
            ...
            f'font-family="{ff}" font-size="{fs:.0f}" font-weight="600">{eq_section_label}</text>'
        )
    ...
    # Max DD annotation on equity panel (optional)
    ...
    # Drawdown panel section label
    if dd_section_label:
        parts.append(
            f'<text x="{x0}" y="{y0_dd + fs*0.55:.0f}" fill="{C_DD}" '
            f'font-family="{ff}" font-size="{fs:.0f}" font-weight="600">{dd_section_label}</text>'
        )
    ...
    # Max drawdown marker on drawdown panel (optional)
    if show_dd_markers:
        parts.append(
    ...
    # X-axis labels (sparse; edge-aware anchoring)
    ...
    for j, i in enumerate(tick_idx):
        ...
        parts.append(
            ...
            f'font-family="{ff}" font-size="{fs:.0f}">{_fmt_date(dates[i], compact=date_compact)}</text>'
        )
...
<title>Options Strategy — Performance Overview</title>
<style>
  :root {{
    --bg: #070d18;
    --card: #0f1a2b;
...
def main() -> None:
    ...
    # LinkedIn PNGs (1600×900 landscape, 1080×1080 square)
