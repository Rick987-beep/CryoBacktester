...
class CalPremiumCollectMk2:
    ...
    DATE_RANGE = ("2026-03-26", "2026-05-26")
    DESCRIPTION = (
    ...
    def _close_pair(self, state, pos, reason, fees_long=True):
        ...
        Uses the engine-owned `close_position` API; the engine emits close
        fills only, as the legs' open fills were emitted when each leg was
        first added to the position.
        """
        md = pos.metadata
        is_call = md["is_call"]
